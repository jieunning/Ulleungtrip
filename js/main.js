...
// ── AI 비서 카드 ──
function aiReply(q) {
  if (/코스|일정|여행지|어디/.test(q)) return '처음이시라면 <b>2박 3일 핵심 일주 코스</b>를 추천드려요. 모노레일·케이블카·관음도·나리분지까지 담겨 있어요. <a href="courses.html">추천 코스 보러 가기 →</a>';
  if (/배|뱃|선박|시간표|결항|출항|여객/.test(q)) return '포항·강릉·묵호·후포에서 울릉도행 여객선이 출발해요. 포항 쾌속선 기준 약 3시간 30분이에요. 운항 여부는 기상에 따라 달라지니 예매 전 선사 공지를 확인하세요. <a href="info.html">뱃길 안내 →</a>';
  if (/독도/.test(q)) return '오늘 독도 접안 지수는 <b>보통</b>이에요. 독도 여객선은 저동항에서 출발하고, 기상에 따라 접안이 어려울 수 있어 일정 중반 배치를 추천드려요. <a href="weather.html">날씨 대시보드 →</a>';
  if (/맛집|먹|음식|식당|홍합|칼국수|특산/.test(q)) return '울릉도에 오셨다면 <b>홍합밥</b>, <b>따개비칼국수</b>, <b>약소불고기</b>는 꼭 드셔보세요. <a href="food.html">먹거리·특산물 →</a>';
  if (/숙소|호텔|펜션|민박|잘 곳/.test(q)) return '도동·저동 시가지 숙소는 이동이 편하고, 북면 오션뷰 펜션은 전망이 좋아요. <a href="stay.html">숙박 가이드 →</a>';
  if (/날씨|기상|파고/.test(q)) return '오늘 울릉은 26° 구름 조금, 파고 0.8m로 여객선 정상 운항 중이에요. 주간 예보와 해상 정보는 대시보드에서 확인하세요. <a href="weather.html">날씨 대시보드 →</a>';
  return '<b>코스·배편·독도·맛집·숙소·날씨</b>에 대해 물어봐 주세요!';
}
...
function aiAsk(q) {
  var home = document.getElementById('aiInput');
  q = q || (home ? home.value.trim() : '') || document.getElementById('chatInput').value.trim();
  chatOpen();
  if (home) home.value = '';
...
// ── 맛집·카페 상세 DB & 렌더 ──
var PLACE_DB = {
  sinbi: { n:'신비섬횟집', cat:'물회 · 전복죽', area:'도동', img:'images/place-sinbi.jpg', r:'4.6',
  ...
  narichon: { n:'나리촌식당', cat:'산채요리', area:'나리분지', img:'images/place-narichon.jpg', r:'4.7',
  ...
  taeyang: { n:'태양식당', cat:'따개비칼국수', area:'저동', img:'images/place-taeyang.jpg', r:'4.5',
  ...
  yakso: { n:'약소불고기 명가', cat:'한우구이', area:'도동', img:'images/place-yakso.jpg', r:'4.6',
  ...
  hoecenter: { n:'저동항 회센터', cat:'물회 · 활어회', area:'저동항', img:'images/place-hoecenter.jpg', r:'4.4',
  ...
  theham: { n:'오션뷰 카페 더함', cat:'대형카페', area:'북면 죽암', img:'images/place-theham.jpg', r:'4.4',
  ...
  ulla: { n:'카페울라', cat:'오션뷰 대형카페', area:'천부', img:'images/place-ulla.jpg', r:'4.5',
  ...
  jeodong: { n:'저동커피', cat:'로스터리 카페', area:'저동항', img:'images/place-jeodong.jpg', r:'4.4',
  ...
  glim: { n:'카페글림', cat:'디저트 카페', area:'도동', img:'images/place-glim.jpg', r:'4.5',
  ...
  supknick: { n:'숲크닉커피', cat:'피크닉 카페', area:'현포', img:'images/place-supknick.jpg', r:'4.3',
